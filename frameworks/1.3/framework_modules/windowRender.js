...
function compileHtmlCss(window, windowVars, ejsHtml, scss, callback){
	...
	sass.render(scss, function(err, css){
		...
		css = css.replace(/url\('\//g, "url('/app/" + process.pid + '/w_' + window.id + '/');
...
function loadFramework(windowData, callback){
	...
	async.parallel([
		function(cb){
			fs.readFile(__dirname + '/../localCode/windowObj.js', cb);
		},
		function(cb){
			fs.readFile(__dirname + '/../localCode/framework.js', cb);
		},
...
function renderWindow(windowPath, window, callback){
	loadWindowFiles(windowPath, function(files){
		...
		fs.readFile(__dirname + '/../localCode/'+window._template, function(err, templateCode){
			if(err) { callback(err, null); return; }
			fs.readFile(__dirname + '/../localCode/style.scss', function(err, templateStyle){
				if(err) { callback(err, null); return; }
				loadFramework(windowData, function(javascriptCode){
					var cssCode = templateStyle + ' .windowContent { ' + files.scripts.scss + ' }';
					compileHtmlCss(window, window.vars, files.scripts.ejs, cssCode, function(GUIcode){
						var windowCode = ejs.render(templateCode.toString(), {
							windowID:window.id,
							GUIcontent:GUIcode,
							javascript:javascriptCode
						});//ejs.render
						callback(null, windowCode);
					});//compileHtmlCss
				});//load framework
			});//loadResource
		});//loadResource
