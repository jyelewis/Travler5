...
function renderWindow(windowPath, window, callback){
	loadWindowFiles(windowPath, function(files){
		...
		loadResource('/localCode/windowTemplate.ejs', function(err, data){
			loadFramework(windowData, function(javascriptCode){
				compileHtmlCss(window.id, window.vars, files.scripts.ejs, files.scripts.scss, function(GUIcode){
					var windowCode = ejs.render(data.toString(), {
						windowID:window.id,
						GUIcontent:GUIcode,
